...
<div class="py-6">
  <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
    <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
      <h1 class="text-2xl font-semibold mb-4 text-gray-900 dark:text-gray-100">Tus pedidos</h1>
            ...
            <div class="flex justify-between items-center">
              <div>
                ...
                <div class="text-sm text-gray-500 dark:text-gray-400">
                  Realizado: {{ $order->created_at->format('d/m/Y H:i') }}
                </div>
              </div>
              @php
                $statusLabel = [
              ...
              <div class="flex items-center gap-2">
                ...
                @if($order->status === 'en_envio')
                  <form method="POST" action="{{ route('orders.updateStatus', $order) }}" onsubmit="return confirm('Confirmar recepción del pedido?');">
                    ...
                    <button class="px-3 py-1 rounded bg-emerald-600 text-white text-sm">Recibido</button>
                  </form>
                @endif
